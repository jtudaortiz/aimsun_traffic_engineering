...
def fillattribute(column):
	sectionType = model.getType ("GKSection")
	con_delay_col = model.getColumn("DYNAMIC::SRC_GKSection_delayTime_0") 
	index = 0 #Modificar el valor para cambiar el intervalo de toma de datos (0= Primer intervalo)		
	...
	for section in GK.GetObjectsOfType (sectionType) :
		...
		if con_delayTS:
			...
			con_delay = section.getDataValueTS(con_delay_col).getAggregatedValue() #getMin()  getMax(0)   getMean(0)
